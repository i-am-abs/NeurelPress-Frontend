              ...
              {article.books.map((book) => (
                <div key={book.id} className="flex gap-3 rounded-lg p-3 hover:bg-muted">
                  {book.coverUrl ? (
                    ...
                    <div className="flex h-16 w-12 items-center justify-center rounded bg-muted text-xs">
                      Cover
                    </div>
                  )}
                  <div>
...
function renderMarkdown(content: string): string {
  // In production, use a proper server-side markdown renderer.
  // For now, return raw content with basic HTML escaping reversed.
  return content
    ...
    .replace(/```(\w+)?\n([\s\S]*?)```/g, '<pre class="bg-neural-900 border border-border rounded-lg p-4 overflow-x-auto my-6"><code class="text-sm font-mono">$2</code></pre>')
